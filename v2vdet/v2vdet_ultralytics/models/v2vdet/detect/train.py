...
class v2vWorldTrainer(DetectionTrainer):
  """
  ...
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    ...
    if hasattr(self.args, 'argum_dict'):
      if self.args.argum_dict == -1:
        self.argum_dict = {
        ...
        self.argum_dict = self.args.argum_dict
    else:
      self.argum_dict = {
                  'rotation_range': (-30, 30),  
                  'scale_range': (0.8, 1.2),    
  ...
  def _setup_train(self, world_size):
    ...
    if RANK in {-1, 0}:
      ...
      # )
      self.test_loader = self.get_dataloader(
          self.testset, batch_size=batch_size if self.args.task == "obb" else eval_batch_size, rank=-1, mode="val"
      )
...
class v2vTrainer(DetectionTrainer):
  """
...
class V2V_New_Dataset_Method_Trainer(v2vWorldTrainer):
  ...
  def build_dataset(self, img_path, mode="train", batch=None):
    ...
    """
    gs = max(int(de_parallel(self.model).stride.max() if self.model else 0), 32)
    return build_v2v_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs)  
...
class V2V_Template_YOLO_Backbone_Share_Param_new_Segmentation_Dataset_Trainer(V2V_New_Dataset_Method_Trainer):
...
class V2V_Template_YOLO_Backbone_Model_Contrastive_Loss_Trainer(v2vWorldTrainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class v2v_DINO_multi_scale_Trainer(v2v_DINO_Trainer):
...
class v2v_DINO_with_registers_Trainer(v2vWorldTrainer):
  """
  A class to fine-tune a world model on a close-set dataset.
...
class v2v_DINO_with_registers_multi_scale_Trainer(v2v_DINO_with_registers_Trainer):
...
class V2V_template_SigLIP_Trainer(v2vWorldTrainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_template_SigLIPv2_Trainer(V2V_template_SigLIP_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_template_SigLIPv2_multi_scale_Trainer(V2V_template_SigLIPv2_Trainer):
...
class new_V2V_template_SigLIP_Trainer(V2V_New_Dataset_Method_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_template_SigLIP_new_dataset_Trainer(V2V_template_SigLIP_Trainer):
...
class V2V_template_SigLIP_multi_scale_Trainer(V2V_template_SigLIP_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_template_SigLIP_multi_scale_multi_head_Trainer(V2V_template_SigLIP_multi_scale_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_With_MultiScale_SAVPE_Trainer(v2vWorldTrainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
  ...
  def preprocess_batch(self, batch):
    ...
    batch = super(v2vWorldTrainer, self).preprocess_batch(batch=batch)
    # batch = super().preprocess_batch(batch=batch)
    # NOTE: add template matching features
  ...
  def get_validator(self):
      ...
      return v2v_with_SAVPE_DetectionValidator(
          test_loader, save_dir=self.save_dir, args=copy_args, _callbacks=self.callbacks
      )
...
class V2V_With_MultiScale_SAVPE_SigLIP2_B_Trainer(V2V_With_MultiScale_SAVPE_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_With_MultiScale_SAVPE_SigLIP2_L_Trainer(V2V_With_MultiScale_SAVPE_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
...
class V2V_With_MultiScale_SAVPE_PE_B16_Trainer(V2V_With_MultiScale_SAVPE_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
  ...
  def build_dataset(self, img_path, mode="train", batch=None):
    ...
    if mode != "train":
      gs = max(int(de_parallel(self.model).stride.max() if self.model else 0), 32)
      dataset = build_object_oriented_yolo_dataset(self.args, img_path, batch, self.data, mode=mode, rect=False, stride=gs)
    else:
      # dataset = super().build_dataset(img_path, mode, batch)
      dataset = build_object_oriented_yolo_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val")
    # return build_yolo_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs)
...
class V2V_With_MultiScale_SAVPE_PE_L14_Trainer(V2V_With_MultiScale_SAVPE_PE_B16_Trainer):
  def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
    """Initialize a WorldTrainer object with given arguments."""
    if overrides is None:
      overrides = {}
