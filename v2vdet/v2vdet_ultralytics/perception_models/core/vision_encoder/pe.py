...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        x: torch.Tensor,
        attn_mask: Optional[torch.Tensor] = None,
        output_hidden_list: list = [],
        layer_idx: int = -1,
    ):
        stop_idx = (self.layers + layer_idx) % self.layers
        
        for output_hidden_idx, output_hidden_num in enumerate(output_hidden_list):
            if (output_hidden_num<0):
                output_hidden_list[output_hidden_idx] = self.layers + output_hidden_num
        
        list_output_hidden_layers = []
        for i, r in enumerate(self.resblocks):
            ...
            if output_hidden_list is not [] and (i) in output_hidden_list:
                list_output_hidden_layers.append(x)

            if i == stop_idx:
                break
        
        if output_hidden_list is not []:
            return x, list_output_hidden_layers
...
class VisionTransformer(nn.Module):
    ...
    def _pool(self, x: Union[torch.Tensor, dict]):
        ...
        if isinstance(x, torch.Tensor):
            ...
            if self.pool_type == "tok":
                return {'x': x["x"][:, 0], "hidden_states": x["hidden_states"]}
            elif self.pool_type == "avg":
                return {'x': x["x"].mean(dim=1), "hidden_states": x["hidden_states"]}
            elif self.pool_type == "attn":
                return {'x': self.attn_pool(x["x"]).squeeze(1), "hidden_states": x["hidden_states"]}
            elif self.pool_type == "none":
                return x
            else:
                raise NotImplementedError
            ...
            raise TypeError(f"Expected input to be of type torch.Tensor or dict, but got {type(x)}")
    ...
    def forward_features(
        ...
        if output_hidden_list is not []:
            x, list_output_hidden_states = self.transformer(x, layer_idx=layer_idx, output_hidden_list=output_hidden_list)
        else:
            x = self.transformer(x, layer_idx=layer_idx)

        if norm:
            if output_hidden_list is not []:
                output_hidden_states_tensor = torch.stack(list_output_hidden_states, 0)
                output_hidden_states_tensor = self.ln_post(output_hidden_states_tensor)
            x = self.ln_post(x)


        if strip_cls_token and output_hidden_list is not []:
            output_hidden_states_tensor = output_hidden_states_tensor[:, :, 1:, :]
        ...
        if output_hidden_list is not []:
            return {'x': x,
                    'hidden_states': output_hidden_states_tensor}
        else:
            return x
