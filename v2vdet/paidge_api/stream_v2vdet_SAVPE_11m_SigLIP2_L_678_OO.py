...
# CKPT_NAME = 'ckpt/Object_Oriented_CKPT/OO_v11m_SAVPE_SigLIP2_multi_layer_135.pt'
CKPT_NAME = "v2v_training_result/from_H100/SigLIP2_series/20250519/OO_v11m_SAVPE_SigLIP2_L_FT_multi_layer_678/weights/best.pt"
...
class v2vdet_class():
    ...
    def set_classes_multi_view(self, query_img: [str], query_class_name: [str]):
        ...
        self.model.model.multi_view_inference_set_classes(self.query_img_emb, bboxes, valid_class_name)
    ...
    #     # Load corresponding mask files, apply masks, and convert to bounding boxes
    #     bboxes_list = []
    #     masked_img_emb = []
    #     for img_idx, (img_path, img) in enumerate(zip(query_img, self.query_img_emb)):
    ...
    #             raise ValueError(f"Mask dimensions {mask.shape} do not match image dimensions {img_array.shape[:2]} for {img_path}")
            
    #         # Apply mask to set background to black
    #         masked_array = img_array.copy()
    #         masked_array[mask == 0] = [0, 0, 0]  # Set background pixels to black
    #         masked_img = Image.fromarray(masked_array)
    #         masked_img_emb.append(masked_img)
            
    #         # Find contours to compute bounding box
    #         contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    #         if not contours:
    #             # If no contours, use a default bounding box (full image)
    #             bbox = [0, 0, mask.shape[1], mask.shape[0]]
    #         else:
    #             # Get the bounding box of the largest contour
    #             x, y, w, h = cv2.boundingRect(contours[0])
    ...
    #                 if w_temp * h_temp > w * h:
    #                     x, y, w, h = x_temp, y_temp, w_temp, h_temp
    #             bbox = [x, y, w, h]  # [top_x, top_y, width, height]
            
    #         bboxes_list.append(bbox)
        
    #     # Update self.query_img_emb with masked images
    #     self.query_img_emb = masked_img_emb
    #     bboxes = torch.tensor(bboxes_list)
...
def detect_items():
    ...
    if not model_instance:
        ...
        query_img_list = [f"/DATA3/uploads/{img}" for img in query_img_list if img.split('_')[-1].startswith('0.')]
        ...
        model_instance.set_classes_multi_view(query_img_list, class_name_list)
