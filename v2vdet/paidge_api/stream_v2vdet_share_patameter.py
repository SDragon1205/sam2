...
from v2vdet.v2vdet_ultralytics.models.v2vdet.model import V2V_Template_YOLO_Backbone, v2vYOLOWorld, V2V_multi_scale_clip, V2V_Template_YOLO_Backbone_Share_Param
import cv2
import pickle
...
class v2vdet_class():
    def __init__(self):
        ...
        self.model = V2V_Template_YOLO_Backbone_Share_Param("v2vdet_ultralytics/cfg/models/v8/yolov8s-world.yaml")
        self.ckpt_path = "ckpt/share_parameter/bb_share_yolov8s_param_Object365_part2_all_1st_stage_training.pt"
        
        '''
        LVIS Training:
        training_result/v2vdet/lvis_Template_YOLO_Backbone_Share_Param_yoloworld_new_teamplate_args_method/weights/best.pt
        
        Object365 Training Part 1 (Finetune from LVIS Training):
    ...
    def set_classes(self, query_img: [str], query_class_name: [str]):
        ...
        self.query_img_emb = [resize_with_padding(Image.open(img).convert(mode="RGB")) for img in self.query_img]
        # self.query_img_emb[0].save('template.jpg')

        self.class_name = query_class_name
        self.model.set_classes(classes=self.class_name, crop_img=self.query_img_emb)
        # self.query_img_emb[-1].save('template.jpg')
