...
class v2vdet_class():
    def __init__(self):
        ...
        # Initialize model
        self.model = V2V_With_MultiScale_SAVPE(MODEL_YAML)
        self.model.info()
    ...
    def set_classes(self, query_img: [str], query_class_name: [str]):
        ...
        # Take the front view of each pic
        # self.query_img_emb = [Image.open(img).convert(mode="RGB") for img in self.query_img]
        
        template_multi_view_img_paths = []
        for i, img_path in enumerate(self.query_img):
        
            
            first_img = np.array(Image.open(img_path))
            height, width, channels = first_img.shape
            
            sum_img = np.zeros((height, width, channels), dtype=np.float64)

            # for j in range(6):
            #     new_view_img_path = img_path.replace("_0.png", f"_{j}.png")
            #     if Path(new_view_img_path).is_file():
            #         template_multi_view_img_path.append(Image.open(new_view_img_path))
                    
            temp_img_number = 0
            for img_path in range(6):
                new_view_img_path = img_path.replace("_0.png", f"_{j}.png")
                if Path(new_view_img_path).is_file():
                    temp_img = np.array(Image.open(new_view_img_path), dtype=np.float64)
                    sum_img += temp_img
                    temp_img_number += 1
            
            avg_img = sum_img / temp_img_number
            avg_img = np.clip(avg_img, 0, 255).astype(np.uint8)
            template_multi_view_img_paths.append(avg_img)
        
        assert len(template_multi_view_img_paths) == len(query_img), f"Length mismatch: class's amount not equal to number of input template imgs. {len(template_multi_view_img_paths)} != {len(query_img)}"
        ...
        for img_path in query_img:
            ...
            for j in range(6):
                new_view_mask_path = mask_path.replace("_0.png", f"_{j}.png")
                if Path(new_view_mask_path).is_file():
        
                    mask = cv2.imread(new_view_mask_path, cv2.IMREAD_GRAYSCALE)
        ...
        bboxes = [torch.tensor(b_list) for b_list in bboxes_list]
