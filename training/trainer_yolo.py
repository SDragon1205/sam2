...
class Trainer_yolo:
    ...
    def _step(
        ...
        #     print(f"Frame {i} keys: {list(frame_output.keys())}")
        # print("self.meters:", self.meters)
    ...
    def _run_step(
        ...
        # }
        # memory_named = {}
        # for i, mem_out in output_dict["non_cond_frame_outputs"].items():
        #     mem_feat = mem_out["maskmem_features"]

        #     # 確保這條路徑留著
        #     loss = loss + 0.0 * mem_feat.sum()

        #     # 轉 leaf tensor (for graphviz 可視化）
        #     memory_named[f"memory_feat_{i}"] = mem_feat.detach().requires_grad_(True)
