...
# ckpt_path = "/home/user/sdragon/sam2/checkpoints/yolo_world_s.pt"
# with g_pathmgr.open(ckpt_path, "rb") as f:
...
state_dict_yolo = DetectionModel(cfg="yolov8s.yaml", ch=3, nc=30, verbose=True)

# for k, v in state_dict_yolo.state_dict().items():  # 第一層
#     print(k)
...
for k, v in state_dict_copy.items():  # 第一層
    if isinstance(v, dict):  # 如果值是字典，進入第二層
        for k1 in v.keys():  # 第二層鍵
            ...
            if "model" in k:  # 假設 "model" 是主要的層
                for param_name, param_value in state_dict_yolo.state_dict().items():
...
output_path = "imagenetvid_yolos_m_num_maskmem_1_memory_position_0_no_mask_downsampler_downsampler_attentionlayer_1.pt"
# 儲存新的模型權重
