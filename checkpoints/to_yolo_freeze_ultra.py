...
# ckpt_path2 = "tt20_yolom_s_mode2_test_best_need_freeze.pt"
ckpt_path2 = "/home/user/sdragon/sam2/sam2_logs/configs/sam2.1_training/tt20_oom11_s_mode2_det.yaml/checkpoints/checkpoint.pt"

with g_pathmgr.open(ckpt_path, "rb") as f:
    state_dict = torch.load(f, map_location="cpu")

with g_pathmgr.open(ckpt_path2, "rb") as f:
    state_dict2 = torch.load(f, map_location="cpu")
...
#             if "model" in k:  # 假設 "model" 是主要的層
#                 for param in state_dict_yolo['model'].parameters():
...
# for name, param in state_dict_copy.named_parameters():
#     if name.startswith("yolo.detection_model."):
#         param.requires_grad = False
#         print(f"Froze: {name}")
prefix = "yolo.detection_model."
# 替換掉 prefix 開頭的 key
    ...
    if isinstance(v, dict):
        for k1, v1 in v.items():
            if k1.startswith(prefix):
                # print(k1)
                # if k in state_dict_copy:
                #     print(f"🔁 替換參數: {k}")
                # else:
                #     print(f"➕ 新增參數: {k}")
                # state_dict_copy[k] = v
                print(f"🔁 替換: {k1}")
                state_dict_copy[k][k1] = v1
...
# output_path = "/home/si2/sdragon/sam2/checkpoints/yolov8s_m_num_maskmem_2_memory_position_0_no_mask_downsampler_downsampler_attentionlayer_1_FREEZE_tt20_yolom_s_mode2_test_best.pt"
# output_path = "/home/user/sdragon/sam2/checkpoints/tt20_oom11_s_mode2_det_freeze_test_best.pt"
output_path = "/home/user/sdragon/sam2/checkpoints/tt20_oom11_s_mode2_det_freeze_test_best_num_maskmem_4.pt"
# 儲存新的模型權重
