...
# ckpt_path = "/home/si2/sdragon/sam2/sam2_logs/configs/sam2.1_training/yolom_s_num_maskmem_1_before_neck_memory_position_0_onlypos_pos_enc_at_attn_num_layers_1_no_rope_k_repeat.yaml/checkpoints/checkpoint.pt"
...
for k, v in state_dict.items():  # 第一層
    if isinstance(v, dict):  # 如果值是字典，進入第二層
        if "model" in k:
            for k1, v1 in v.items():  # 第二層鍵
                # if "model" in k:  # 假設 "model" 是主要的層
                print(k1)
            # # # 過濾掉所有 "image_encoder.xxx" 的參數
            # filtered_v = {k1: v1 for k1, v1 in v.keys}# if not (k1.startswith("state") or k1.startswith("param_groups") or k1.startswith("epoch") or k1.startswith("train") or k1.startswith("val") or k1.startswith("time_elapsed") or k1.startswith("Trainer") or k1.startswith("scale") or k1.startswith("growth_factor") or k1.startswith("backoff_factor") or k1.startswith("growth_interval") or k1.startswith("_growth_tracker"))}
        
            # # 如果 `filtered_v` 變成空的，就直接刪掉 `k`
            # if not filtered_v:
            #     del state_dict_copy[k]
            # else:
                state_dict_copy[k1] = v1
    # else:
    #     print("other:", k)
...
# memory_encoder = MemoryEncoder(out_dim= 128, position_encoding=position_encoding, mask_downsampler=mask_downsampler, fuser=fuser, in_dim=128, only_pos=True)#no_mask_downsampler= True)#, only_pos=True)
...
output_path = "/home/si2/sdragon/sam2/checkpoints/trained_yolom_s_num_maskmem_1_before_neck_memory_position_0_onlypos_pos_enc_at_attn_num_layers_1_no_rope_k_repeat.pt"
