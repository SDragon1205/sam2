...
for k, v in state_dict.items():  # 第一層
    if isinstance(v, dict):  # 如果值是字典，進入第二層
        ...
        filtered_v = {k1: v1 for k1, v1 in v.items() if not (k1.startswith("memory_encoder.mask_downsampler.") or k1.startswith("mask_downsample.") or k1.startswith("no_obj_ptr") or k1.startswith("image_encoder.") or k1.startswith("sam_prompt_encoder.") or k1.startswith("yolo_detection_head.") or k1.startswith("sam_mask_decoder.") or k1.startswith("obj_ptr_proj") or k1.startswith("obj_ptr_tpos_proj.") or k1.startswith("no_obj_embed_spatial"))}
...
memory_encoder = MemoryEncoder(out_dim= 64, position_encoding=position_encoding, mask_downsampler=mask_downsampler, fuser=fuser, in_dim=128)
...
output_path = "/home/si2/sdragon/sam2/checkpoints/yolov8s_m_num_maskmem_1_memory_position_0.pt"
